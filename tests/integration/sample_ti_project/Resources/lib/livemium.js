...
// Not stable : 'width', 'title', 'text', 'layout'
var JSS_PROPERTIES = [
...
var TiObject = (function () {
	...
	return {
		...
		getViewHierarchy: function () {
			...
			// FIXME : navigation group does not provide a native way to get the current window. You may set all window
			// from navgroup manually with the register() method provided by the Watcher.
			// If you implemented a method to access the current window, then you may use registerCustomWindow() and specify a 
			// callback as argument that will be invoked to set the view hierarchy of the window returned by the callback;
			// by the Watcher.
...
function Connection () {
	...
	var _events         = {}
		, _socket         = null
		, _buffer         = ""
		, MAX_CHUNK_SIZE = 1024; // buffer size : 
